...
Views.mentors = function(el){
  ...
  const curId = App.ctx.mentorId || WLN.MENTORS[0].id;
  ...
  <div class="topActions" style="margin-bottom:16px">
    <h2 style="margin:0;flex:1">AI Wayang Mentors</h2>
    <span class="chip ${state.apiKey?'teal':'dim'}">${state.apiKey?'✦ Live AI':'Built-in coach'}</span>
  </div>
  <p class="muted small" style="margin:0 0 18px;max-width:700px;line-height:1.6">
    Choose your wayang guide — write, speak, or paste your English and receive feedback in their unique voice. Each mentor focuses on a different dimension of language mastery.
  </p>

  <!-- ═══ Character Roster ═══ -->
  <div style="margin-bottom:22px">
    <div style="font-size:11px;letter-spacing:.14em;color:var(--gold);text-transform:uppercase;margin-bottom:12px">✦ Panakawan Guides</div>
    <div class="mentorGrid" style="grid-template-columns:repeat(5,1fr);margin-bottom:14px">
      ${WLN.MENTORS.map(m=>`
        <div class="mentorCard ${m.id===cur.id?'active':''}" data-mentor="${m.id}" title="${esc(m.role)}">
          <div class="portraitWrap">${portrait(m)}</div>
          <b>${esc(m.name)}</b>
          <span class="mRole">${esc(m.role)}</span>
          <p>${esc(m.desc)}</p>
        </div>`).join('')}
    </div>
    <div style="font-size:11px;letter-spacing:.14em;color:var(--gold);text-transform:uppercase;margin-bottom:12px">✦ Hero Characters</div>
    <div class="mentorGrid" style="grid-template-columns:repeat(5,1fr)">
      ${WLN.HEROES.map(m=>`
        <div class="mentorCard ${m.id===cur.id?'active':''}" data-mentor="${m.id}" title="${esc(m.role)}">
          <div class="portraitWrap">${portrait(m)}</div>
          <b>${esc(m.name)}</b>
  ...
  <!-- ═══ Chat Studio ═══ -->
  <div class="gridMain" style="gap:20px">

    <!-- Left: portrait + info -->
    <div style="display:flex;flex-direction:column;gap:14px;max-width:320px">
      <div class="panel ornate" style="text-align:center;padding:24px">
        <div class="portraitWrap" style="width:140px;height:140px;margin:0 auto 16px;border-radius:16px;overflow:hidden;border:2px solid var(--gold);background:#070e1e">
          ${portrait(cur)}
        </div>
        <h3 style="font-size:18px;margin:0 0 4px">${esc(cur.name)}</h3>
        <div style="font-size:13px;color:var(--gold);font-weight:600;margin-bottom:10px">${esc(cur.role)}</div>
        <p class="muted small" style="margin:0 0 12px;line-height:1.6">${esc(cur.desc||cur.focus)}</p>
        ${cur.style?`<div style="font-size:11.5px;color:var(--cream-dim);font-style:italic;line-height:1.5;padding:10px;background:rgba(215,169,40,.05);border-radius:8px;border:1px solid var(--line-soft)">Style: ${esc(cur.style)}</div>`:''}
      </div>
      <div class="panel ornate">
        <h3 class="panelTitle">Focus Areas</h3>
        <div style="display:flex;flex-wrap:wrap;gap:6px">
          ${(cur.focus||'').split(',').map(f=>`<span class="chip" style="font-size:11px">${esc(f.trim())}</span>`).join('')}
        </div>
        <div style="margin-top:14px;font-size:12.5px;color:var(--cream-dim);line-height:1.6">
          💡 <b style="color:var(--cream)">Tip:</b> Paste a paragraph from your writing, a speaking transcript, or ask a direct language question.
        </div>
      </div>
    </div>

    <!-- Right: chat interface -->
    <div class="panel ornate" style="display:flex;flex-direction:column;min-height:520px">
      <div class="panelTitle" style="align-items:center">
        <span>${esc(cur.name)}'s Studio</span>
        <div style="display:flex;gap:6px;margin-left:auto">
          <button class="btn small" id="btnPlay" title="Play last response">${Icons.play} Play</button>
          <button class="btn small" id="btnStopTts" title="Stop speech">${Icons.stopIc} Stop</button>
          <button class="btn small" id="btnSaveChat" title="Save conversation">💾 Save</button>
          <button class="btn small" id="btnClearChat" title="Clear history">🗑 Clear</button>
        </div>
      </div>

      <!-- Chat log -->
      <div class="chatLog" id="chatLog" style="flex:1;min-height:220px;max-height:340px">
        ${chat.length
          ? chat.map(msgBubble).join('')
          : `<div class="bubble ai">
               <span class="who">${esc(cur.name)} · ${esc(cur.role)}</span>
               ${esc(cur.opener||'Hello! I am '+cur.name+'. Share your English with me — I am ready to guide you.')}
             </div>`}
      </div>

      <!-- Input area -->
      <div style="margin-top:12px">
        <textarea class="input" id="mentorInput" placeholder="Write a sentence, paste a paragraph, or ask ${esc(cur.name)} a language question…" style="min-height:90px;margin-bottom:10px;width:100%;box-sizing:border-box"></textarea>

        <!-- Action buttons -->
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
          <button class="btn primary" id="btnGetFeedback" style="flex:1;min-width:140px;padding:11px">
            ✦ Get Feedback from ${esc(cur.name)}
          </button>
          <button class="btn" id="btnRecord" style="display:flex;align-items:center;gap:7px;padding:11px 16px">
            ${Icons.mic} <span id="recLabel">Record</span>
          </button>
          <button class="btn small" id="btnListenInput" title="Listen to your text read aloud">🔊 Read Aloud</button>
        </div>
        <div style="margin-top:8px;font-size:12px;color:var(--cream-dim);display:flex;align-items:center;gap:8px">
          <span id="recStatus"></span>
          ${state.apiKey?'<span class="chip teal" style="font-size:10px">Live AI Active</span>':'<span style="color:rgba(203,189,151,.5)">Add API key in Settings for live AI responses</span>'}
        </div>
      </div>
    </div>
  </div>`;
  ...
  /* ── Character switching ── */
  document.querySelectorAll('[data-mentor]').forEach(card=>{
    card.addEventListener('click', ()=>{ App.ctx.mentorId=card.dataset.mentor; Views.mentors(el); });
  });

  /* ── Get Feedback ── */
  document.getElementById('btnGetFeedback')?.addEventListener('click', async ()=>{
    ...
    input.value=''; scrollEnd();

    const thinking = document.createElement('div');
    thinking.className='bubble ai thinking';
    thinking.innerHTML=`<span class="who">${esc(cur.name)}</span><span style="opacity:.6">Thinking…</span>`;
    log.appendChild(thinking); scrollEnd();

    let reply = null;
    if(state.apiKey){
      try{ reply = await Mentor.liveReply(cur, chat, txt, lesson); }catch(e){ reply=null; }
    }
    ...
    log.insertAdjacentHTML('beforeend', msgBubble(chat.at(-1)));
    scrollEnd(); addXP(25,'Mentor consultation'); save();
  });

  /* ── Enter to send (Ctrl+Enter) ── */
  input?.addEventListener('keydown', e=>{
    if((e.ctrlKey||e.metaKey) && e.key==='Enter'){
      document.getElementById('btnGetFeedback')?.click();
    }
  });
  ...
  /* ── Stop TTS ── */
  document.getElementById('btnStopTts')?.addEventListener('click', ()=>Speech.stop());

  /* ── Read input aloud ── */
  document.getElementById('btnListenInput')?.addEventListener('click', ()=>{
    const txt = input?.value?.trim();
    if(txt) Speech.speak(txt);
    else toast('Type something to read aloud.');
  });
  ...
  document.getElementById('btnRecord')?.addEventListener('click', ()=>{
    ...
    if(!Speech.recAvailable){
      toast('Speech recognition not available in this browser. Try Chrome.');
      return;
    }
    if(!isRecording){
      isRecording=true;
      ...
      Speech.startRecognition({
        onPartial: txt=>{ if(input) input.value=txt; },
        onEnd: txt=>{
          isRecording=false;
          if(label) label.textContent='Record';
          if(status) status.textContent=txt?'✓ Voice captured':'(no speech detected)';
          document.getElementById('btnRecord')?.classList.remove('active');
          if(txt && input) input.value=txt;
        },
        onError: ()=>{
          isRecording=false;
          if(label) label.textContent='Record';
          if(status) status.textContent='Microphone error.';
          document.getElementById('btnRecord')?.classList.remove('active');
        }
      });
    } else {
  ...
  document.getElementById('btnSaveChat')?.addEventListener('click', ()=>{
    if(chat.length===0){ toast('No conversation to save yet.'); return; }
    const lines = [
      `Wayang Lingua Nusantara — Mentor Session`,
      `Mentor: ${cur.name} (${cur.role})`,
      `Date: ${new Date().toLocaleString()}`,
      '─'.repeat(50),
      ...chat.map(m=>`[${m.role==='user'?'You':cur.name}]\n${m.text}\n`)
    ];
    download(`WLN_${cur.name}_chat_${Date.now()}.txt`, lines.join('\n'), 'text/plain');
    toast('Conversation saved.');
  });

  /* ── Clear chat ── */
  document.getElementById('btnClearChat')?.addEventListener('click', ()=>{
    if(!confirm('Clear this conversation with '+cur.name+'?')) return;
