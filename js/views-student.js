...
Views.mentors = function(el){
  ...
  const MENTOR_DATA = {
    responses: {
      ...
      gatotkaca:(txt)=>`Intelligibility check: Your message ${txt.length>50?'has sufficient length — good context':'is brief; add more context for clarity'}. Volume and emphasis matter most: stress the KEY word in every sentence (e.g. "Communication is the BRIDGE between cultures"). Aim for 130–150 wpm in formal speech. Clarity score: ${txt.split(' ').filter(w=>w.length>7).length} complex words detected — excellent range. High-flying communication soars when every word lands precisely!`,
